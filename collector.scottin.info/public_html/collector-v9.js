 ...
 * CSE 135 - Module 10: Production Readiness (edited)
 *
...
(function () {
  ...
  // tracking load start and end
  function getLoadInfo() {
    const t = performance.timing || {};
    const loadStart = t.navigationStart || null;
    const loadEnd = t.loadEventEnd || null;

    return {
      loadStart: loadStart,
      loadEnd: loadEnd,
      totalLoadTime: loadStart && loadEnd ? (loadEnd - loadStart) : null
    };
  }
  ...
  function detectCssEnabled() {
    cssEnabled = !!(document.styleSheets && document.styleSheets.length > 0);
  }
  ...
  function initErrorTracking() {
    ...
    // tracking mouse keyboard scroll and idle
  function initActivityTracking() {
    sendActivity('page_enter', {
      enteredAt: enteredAt
    });

    document.addEventListener('mousemove', function (event) {
      const now = Date.now();
      if (now - lastMouseSent < mouseThrottleMs) return;
      lastMouseSent = now;
    ...
    window.addEventListener('scroll', function () {
      const now = Date.now();
      if (now - lastScrollSent < scrollThrottleMs) return;
      lastScrollSent = now;
